import pytest
import numpy as np
from ml.distances.metrics import EuclideanDistance, ascii_word_dist
from ml.supervised_learning.knn import KNN
from ml.pre_processing.pre_process import normalize_by_max_abs, normalize_by_average_abs, train_test_split, normalize_vectors_by_max_abs, normalize_vectors_by_average_abs
...
def test_knn_pipeline_max_abs_numeric_classification():
    ...
    train, test = train_test_split(normalized, train_ratio=0.5, seed=42)
    ...
    assert 0.0 <= err <= 1.0, "Should classify correctly with K=2 after max-abs normalization"
...
def test_knn_pipeline_max_abs_numeric_regression():
    ...
    knn = KNN(X_train, y_train)
    err = knn.error(X_test, y_test, K=2, classify=False)
    assert 0.0 <= err <= 8.0, "Should regress correctly with K=2 after max-abs normalization"
...
def test_knn_pipeline_average_abs_numeric_classification():
    ...
    knn = KNN(X_train, y_train)
    err = knn.error(X_test, y_test, K=2)
    assert 0.0 <= err <= 1.0, "Should classify correctly with K=2 after average-abs normalization"
