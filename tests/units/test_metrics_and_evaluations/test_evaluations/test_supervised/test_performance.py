import numpy as np
import pytest
from ml.metrics_and_evaluations.evaluation.supervised.performance import mean_squared_error, classification_accuracy, NegativeLogLikelihoodBatchConstructor, mean_absolute_error
from ml.utils._errors_and_warnings._general_error_handling import ModelInterfaceError, InputShapeError
from ml.metrics_and_evaluations.metrics.metrics import EuclideanDistance, taxicab_distance, LinfinityDistance, ascii_word_dist
...
def test_nll_basic_case():
    NLL = NegativeLogLikelihoodBatchConstructor()
    Y_true = np.array([[0,1,0],[1,0,0]])
    Y_pred = np.array([[0.2,0.7,0.1],[0.9,0.05,0.05]])
    losses = NLL(Y_true, Y_pred)
    assert losses.shape == (2,)
    assert np.all(losses >= 0)
    assert np.isclose(losses[0], -np.log(0.7), atol=1e-6)

def test_nll_uniform_distribution():
    NLL = NegativeLogLikelihoodBatchConstructor()
    Y_true = np.array([[1,0,0],[0,1,0]])
    Y_pred = np.array([[1/3,1/3,1/3],[1/3,1/3,1/3]])
    losses = NLL(Y_true, Y_pred)
    assert np.allclose(losses, -np.log(1/3))

def test_nll_with_float32():
    NLL = NegativeLogLikelihoodBatchConstructor()
    Y_true = np.array([[0,1],[1,0]], dtype=np.float32)
    Y_pred = np.array([[0.4,0.6],[0.8,0.2]], dtype=np.float32)
    losses = NLL(Y_true, Y_pred)
    assert losses.dtype == float
    assert np.all(losses >= 0)

def test_nll_with_float64():
    NLL = NegativeLogLikelihoodBatchConstructor()
    Y_true = np.array([[0,1],[1,0]], dtype=np.float64)
    Y_pred = np.array([[0.4,0.6],[0.8,0.2]], dtype=np.float64)
    losses = NLL(Y_true, Y_pred)
    assert losses.dtype == float
    assert np.all(losses >= 0)

def test_nll_with_negatives_in_y_true():
    # Not realistic, but ensures numeric handling
    NLL = NegativeLogLikelihoodBatchConstructor()
    Y_true = np.array([[0,-1],[1,0]])
    Y_pred = np.array([[0.5,0.5],[0.9,0.1]])
    losses = NLL(Y_true, Y_pred)
    # Negative label will produce negative contribution
    assert np.any(losses < 0)
...
def test_nll_epsilon_nonpositive_raises():
    with pytest.raises(ValueError, match="epsilon must be positive"):
        NegativeLogLikelihoodBatchConstructor(epsilon=0)

def test_nll_shape_mismatch_raises():
    NLL = NegativeLogLikelihoodBatchConstructor()
...
def test_nll_with_nan_in_y_true_raises():
    NLL = NegativeLogLikelihoodBatchConstructor()
    Y_true = np.array([[np.nan,1],[0,1]])
    Y_pred = np.array([[0.5,0.5],[0.4,0.6]])
    with pytest.raises(ValueError):
        NLL(Y_true, Y_pred)

def test_nll_with_nan_in_y_pred_raises():
    NLL = NegativeLogLikelihoodBatchConstructor()
    Y_true = np.array([[1,0],[0,1]])
    Y_pred = np.array([[np.nan,0.5],[0.4,0.6]])
    with pytest.raises(ValueError):
        NLL(Y_true, Y_pred)

def test_nll_non_numeric_input_raises():
    NLL = NegativeLogLikelihoodBatchConstructor()
...
def test_nll_zero_probability_safe_with_epsilon():
    NLL = NegativeLogLikelihoodBatchConstructor(epsilon=1e-8)
    Y_true = np.array([[1,0],[0,1]])
...
def test_nll_all_zero_predictions():
    NLL = NegativeLogLikelihoodBatchConstructor()
    Y_true = np.array([[1,0],[0,1]])
...
def test_mae_basic_float_inputs():
    class Dummy:
        def predict(self, X): return X
    X = np.array([[1.0],[2.0]])
    y = np.array([[1.0],[3.0]])
    mae = mean_absolute_error(Dummy(), X, y)
...
def test_mae_custom_distance():
    class Dummy:
        def predict(self, X): return X
    def custom_distance(a, b):
        return np.sum(np.abs(a - b), axis=1) * 2  # exaggerate
    X = np.array([[1],[2]])
    y = np.array([[1],[3]])
    mae = mean_absolute_error(Dummy(), X, y, distance=custom_distance)
    assert np.isclose(mae, 1.0)  # doubled from 0.5

def test_mae_invalid_distance_type():
    class Dummy:
        def predict(self, X): return X
    with pytest.raises(TypeError):
        mean_absolute_error(Dummy(), [[1]], [[1]], distance="not_callable")

# ---------- Sample weights ----------

def test_mae_with_sample_weights():
    class Dummy:
        def predict(self, X): return X
    X = np.array([[1],[2]])
    y = np.array([[1],[3]])
    sw = np.array([2,1])
    mae = mean_absolute_error(Dummy(), X, y, sample_weight=sw)
    # Weighted average: (0*2 + 1*1)/3 = 0.333...
    assert np.isclose(mae, 1/3)

def test_mae_invalid_sample_weight_shape():
    class Dummy:
        def predict(self, X): return X
    X = np.array([[1],[2]])
    y = np.array([[1],[3]])
    sw = np.array([[1,2]])  # wrong shape
    with pytest.raises(InputShapeError):
        mean_absolute_error(Dummy(), X, y, sample_weight=sw)

def test_mae_zero_sum_sample_weight():
    class Dummy:
        def predict(self, X): return X
    X = np.array([[1],[2]])
    y = np.array([[1],[3]])
...
def test_mae_shape_mismatch_leading_axis():
    class Dummy:
        def predict(self, X): return X
    X = np.array([[1],[2],[3]])
    y = np.array([[1],[2]])
    with pytest.raises(InputShapeError):
        mean_absolute_error(Dummy(), X, y)

def test_mae_shape_mismatch_per_sample():
    class Dummy:
        def predict(self, X): return np.array([[1,2],[3,4]])
    X = np.array([[1],[2]])
    y = np.array([[1],[2]])
    with pytest.raises(InputShapeError):
        mean_absolute_error(Dummy(), X, y)

def test_mae_nan_in_inputs():
    class Dummy:
        def predict(self, X): return X
    X = np.array([[np.nan],[1]])
    y = np.array([[0],[1]])
...
def test_mae_nan_in_predictions():
    class Dummy:
        def predict(self, X): return np.array([[np.nan],[1]])
    X = np.array([[0],[1]])
    y = np.array([[0],[1]])
    with pytest.raises(ValueError):
        mean_absolute_error(Dummy(), X, y)

def test_mae_distance_returns_negative():
    class Dummy:
        def predict(self, X): return X
    ...
    y = np.array([[1],[3]])
    with pytest.raises(ValueError):
        mean_absolute_error(Dummy(), X, y, distance=bad_distance)

def test_mae_distance_returns_non_numeric():
    class Dummy:
        def predict(self, X): return X
    def bad_distance(a, b):
        return ["a","b"]
